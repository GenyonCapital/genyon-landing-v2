    ...
    <main className="min-h-screen">
      ...
      <section className="relative min-h-[92vh] gradient-hero overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 pt-28">
          <div className="flex flex-col items-center text-center gap-6">
            ...
            <div className="mt-10 w-full max-w-4xl rounded-2xl overflow-hidden border border-white/10 bg-[var(--card)]">
              <video autoPlay loop muted playsInline className="w-full h-[420px] object-cover opacity-[.92]">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-[var(--muted)]">
              A GENYON combina bioativos de elevada pureza com modelos de IA para otimizar resposta
              metabólica, saciedade e inflamação de baixo grau.
            </p>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ['Saciedade inteligente','Regulação do apetite'],
              ['Metabolismo ativo','Oxidação lipídica'],
              ['Inflamação baixa','Homeostase intestinal']
            ].map(([title,sub],i)=>(
      ...
      <section className="mx-auto max-w-6xl px-6 pb-12">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[
            { name:'SLIMPRO', desc:'Gestão do apetite, metabolismo de lípidos, energia estável.', bullets:['Menos fome','Menos cravings','Energia suave'] },
            { name:'RESETPRO', desc:'Reset digestivo, microbiota, redução de inchaço e cravings.', bullets:['Microbiota','Desinchaço','Conforto'] }
          ].map((p,i)=>(
            <motion.div key={i} initial={{opacity:0, y:14}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
              ...
              <div>
                ...
                <p className="text-[var(--muted)] mt-1">{p.desc}</p>
                <ul className="mt-3 text-sm text-[var(--muted)] list-disc pl-5">
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ['74%','menor fome em 2 semanas'],
            ['68%','menos cravings à noite'],
            ['3–5 cm','média de perímetro abdominal em 30 dias*']
          ].map(([kpi,txt],i)=>(
            <motion.div key={i} initial={{opacity:0,scale:.96}} whileInView={{opacity:1,scale:1}} viewport={{once:true}}
              className="card p-6 text-center">
              <div className="text-5xl font-bold">{kpi}</div>
              <div className="text-[var(--muted)]">{txt}</div>
            </motion.div>
          ))}
        </div>
        <p className="text-xs text-[var(--muted)] mt-3">
          *Dados indicativos de testes internos piloto. Não substitui aconselhamento médico.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <motion.blockquote initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true}}
          className="text-2xl md:text-3xl leading-tight text-center">
      ...
      <section id="cta" className="mx-auto max-w-4xl px-6 pb-28">
        <div className="card p-8 text-center">
          ...
          <form action="https://formspree.io/f/xbjwzqyz" method="POST" className="flex flex-col sm:flex-row gap-3 justify-center">
            <input name="email" required placeholder="O teu melhor e-mail"
              className="px-4 py-3 rounded-lg bg-transparent border border-white/15 outline-none"/>
        ...
        <div className="mx-auto max-w-6xl px-6 py-10 grid md:grid-cols-2 gap-6">
          <div className="text-sm text-[var(--muted)]">
            <div className="flex gap-3 mb-2">
              <span className="badge">GMP</span>
              <span className="badge">ISO 22000</span>
              <span className="badge">HACCP</span>
              <span className="badge">EU • US • CA • AU</span>
            </div>
            Suplemento alimentar. Não substitui um regime equilibrado. Consulte profissionais de saúde quando necessário.
          </div>
          <div className="text-sm md:text-right text-[var(--muted)]">
            © {new Date().getFullYear()} Genyon Group — Todos os direitos reservados · <a href="#" className="underline">Políticas</a>
          </div>
        </div>
      </footer>
    </main>
